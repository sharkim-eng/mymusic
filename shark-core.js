(() => {
  ...
  const loadSaved=()=>{try{const v=JSON.parse(localStorage.getItem('sharkPlaylist')||'[]');return Array.isArray(v)?v:[]}catch{return[]}};
  const saveSaved=v=>localStorage.setItem('sharkPlaylist',JSON.stringify(v));
  const toggleSaved=s=>{
    let a=loadSaved();
    const k=key(s),i=a.findIndex(x=>key(x)===k);
    if(i>=0)a.splice(i,1);
    else a.push({title:s.title,artist:s.artist,videoId:videoId(s),url:url(s)});
    saveSaved(a);
    return a;
  };
  const isSaved=s=>loadSaved().some(x=>key(x)===key(s));
  const open=s=>{const u=url(s);if(u) location.href=u};
  ...
  const five=(mood,genre)=>{
    ...
    const H='sharkRecommendationHistoryV5';
    const L='sharkLastRecommendationsV5';
    const id=`${mood}_${genre}`;
    const hist=loadObj(H),last=loadObj(L),seen=new Set(hist[id]||[]);
    const need=Math.min(5,pool.length);

    let fresh=pool.filter(s=>!seen.has(key(s)));
    if(fresh.length<need){
      const lastSet=new Set(last[id]||[]);
      const alt=pool.filter(s=>!lastSet.has(key(s)));
      fresh=alt.length>=need?alt:[...pool];
      seen.clear();
    }

    const selected=shuffled(fresh).slice(0,5);
    selected.forEach(s=>seen.add(key(s)));
    hist[id]=[...seen];
    last[id]=selected.map(key);
    sessionStorage.setItem(H,JSON.stringify(hist));
    sessionStorage.setItem(L,JSON.stringify(last));
    return selected;
  };

  window.SHARK={emoji,message,key,videoId,url,loadSaved,saveSaved,toggleSaved,isSaved,open,five};
})();
